...
# Configure app.toml
sed -i 's/pruning = ".*"/pruning = "nothing"/' $HAQQD_DIR/config/app.toml
sed -i 's/pruning-keep-recent = ".*"/pruning-keep-recent = "0"/' $HAQQD_DIR/config/app.toml
sed -i 's/pruning-keep-every = ".*"/pruning-keep-every = "0"/' $HAQQD_DIR/config/app.toml
sed -i 's/pruning-interval = ".*"/pruning-interval = "0"/' $HAQQD_DIR/config/app.toml
sed -i 's/enable-indexer = .*/enable-indexer = true/' $HAQQD_DIR/config/app.toml
...
sed -i 's/indexer = ".*"/indexer = "kv"/' $HAQQD_DIR/config/config.toml
